...
void testSocketIOBlocking(const std::string &filename)
{
	...
	std::cout << "Time taken to write to socket: " << elapsedWrite.count() << " seconds\n";
	...
	std::cout << "Time taken to read from socket: " << elapsedRead.count() << " seconds\n";
...
bool testSocketIO(const std::string &filename)
{
	...
	while (totalBytesWritten < actualFileSize || totalBytesRead < actualFileSize)
	{
		if (totalBytesWritten < actualFileSize)
		{
			std::cout << "Writing to socket\n";
			std::cout << "Data length to write: " << actualFileSize - totalBytesWritten << std::endl;
			if (MSG_DONTWAIT_ON)
			{

				written =
					send(sv[0], buffer.data() + totalBytesWritten, actualFileSize - totalBytesWritten, MSG_DONTWAIT_ON);
			}
			else
			{
				written = send(sv[0], buffer.data() + totalBytesWritten, actualFileSize - totalBytesWritten, 0);
			}
		...
		if (totalBytesRead < actualFileSize)
		{
			...
			if (MSG_DONTWAIT_ON)
			{
				...
				read =
					recv(sv[1], readBuffer.data() + totalBytesRead, actualFileSize - totalBytesRead, MSG_DONTWAIT_ON);
			}
			else
			{
				read = recv(sv[1], readBuffer.data() + totalBytesRead, actualFileSize - totalBytesRead, 0);
			}
...
void createLargeFile(const std::string &filename, std::size_t sizeInMB)
{
	...
	else
	{
		...
		double fileSizeMB = static_cast<double>(fileSizeBytes) / (1024 * 1024); // Convert to MB
...
int main()
{
	...
	if (file.read(buffer.data(), fileSize))
	{
		...
		std::cout << "Time taken to read file: " << elapsed.count() << " seconds.\n";
	}
	else
	{
		std::cerr << "Error reading the file.\n";
